...
used_capacity_threshold = 80
# Maximum allowable capacity for the storage pool (in GiB)
max_storage_pool_capacity = 2048  # Example: 10 TiB limit
# List of region suffixes to try if volume_name is not found in the base region
region_suffixes = ['-a', '-b', '-c', '-d', '-e', '-f']
...
def netapp_volumes_autogrow(cloud_event):
    ...
    # Try to get the volume from the main region and fallback regions
    volume_found = False
    for region_suffix in [''] + region_suffixes:
        current_region = myregion + region_suffix
        volume_name = f"projects/{myproject}/locations/{current_region}/volumes/{myvolume}"

        # Initialize request argument(s)
        request = netapp_v1.GetVolumeRequest(name=volume_name)

        try:
            # Attempt to make the request
            response = client.get_volume(request=request)
            volume_found = True
            print(f"Volume found in region: {current_region}, Volume Name: {volume_name}")
            break  # Exit loop if volume is found
        except Exception as e:
            print(f"Volume not found in region {current_region}. Trying next region...")

    if not volume_found:
        print(f"Volume {myvolume} not found in any regions. Exiting function.")
        return  # Exit the function if volume is not found in any region
    ...
    storagepool_name = f"projects/{myproject}/locations/{current_region}/storagePools/{mystoragepool}"
